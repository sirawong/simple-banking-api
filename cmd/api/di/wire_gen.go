...
func InitializeApp(log *logger.Logger) (*server.App, func(), error) {
	...
	gormDB, cleanup, err := adapterdb.ProvideDB(configConfig)
	if err != nil {
		return nil, nil, err
	}
	...
	service := account.ProvideService(accountRepository, repository)
	...
	accountHandler := handler.ProvideAccountHandler(service, transactionService)
	transactionHandler := handler.ProvideTransactionHandler(transactionService)
	engine := handler2.ProvideRouter(accountHandler, transactionHandler, log)
